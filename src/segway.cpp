...
//      m_rowGyroData  += (m_brick->gyroscope()->read()[m_gyroAxis] - m_gyroDrift)*tmpElapsed*parToDeg/1000.0;
    m_dbgTicker.restart();
    ...
    if (m_cnt == 20) {
      qDebug("angle speed: %1.5f yaw: %d, acc: %2.3f, gyr: %2.3f, pidc: %2.3f %2.3f %2.3f %2.4f", angle, yaw, acceData, m_rowGyroData, m_pk, m_ik, m_dk, m_ck);
      m_cnt = 0;
    }
    m_cnt++;
}
