...
Omni::Omni(QThread *guiThread, QString configPath):
...
void Omni::onGamepadPadDown(int padNum, int vx, int vy)
{
  if (padNum != 1) return;
...
void Omni::onGamepadPadUp(int padNum)
{
  if (padNum != 1) return;
...
void Omni::onGamepadButtonChanged(int buttonNum, int state)
{
...
void Omni::onBrickButtonChanged(int buttonCode, int state)
{
  if (state == 0) return;
...
void Omni::setLineColorData(int hue, int hueTol, int sat, int satTol, int val, int valTol)
{
...
void Omni::setLineTargetData(int x, int angle, int mass)
{
